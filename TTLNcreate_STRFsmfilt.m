...
maxtempmod = 40; % Hz
maxfreqmod = 4; % cycles/octave
nLags = 26;
dt = 2; % time bin in msec
...
clf;
% subplot(2,2,1)
% imagesc(stimP.Lags,stimP.Freqs,DMRCCfilt);
%         set(gca,'xdir','reverse','ydir','normal','clim',max(abs(DMRCCfilt(:)))*[-1 1]);
% 
subplot(2,2,2)
...
subplot(2,2,4)
hold on;
plot(stimP.Freqs,DMRCCfilt(:,centLagidx));
plot(stimP.Freqs,freqfilt);
...
save('SRFsmooth.mat','stimP','lagfilt','STRFsmfilt','-append');
