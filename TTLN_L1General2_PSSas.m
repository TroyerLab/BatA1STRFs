function [w f flag optout] = TTLN_L1General2_AS(funObj,w,lambda,options)
% [w val flag optout] = TTLN_L1General2_AS(funObj,w,lambda,options)
% optimization with L1 regularization
% code by Mark Schmidt
% edited by Todd Troyer to report value, flag for stopping criterion, and
% information about optimization in structure optout
...
for i = 1:maxIter
    ...
    gtd = g'*d;
    % gtd = g*d; %% size fix Todd Troyer 1/1/25
    if gtd > -progTol
        if verbose
    ...
    if  abs(f-f_old) < progTol
        if verbose
        % fprintf('Progress in parameters or objective below progTol\n');
        fprintf('Progress objective below progTol\n');
        end
        flag = 1;
        break;
    end
...
optout.iterations = i;
optout.funcCount = funEvals;
optout.valvec = valvec;
end
